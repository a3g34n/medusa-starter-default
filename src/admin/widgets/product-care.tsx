...
const ProductCareWidget = ({
  data: product,
}: DetailWidgetProps<HttpTypes.AdminProduct>) => {
  const queryClient = useQueryClient()
  const [open, setOpen] = useState(false)
  const initial = (product.metadata?.care_instructions as string) ?? ""
  ...
  const updateMutation = useMutation({
    mutationFn: (value: string) =>
      sdk.admin.product.update(product.id, {
        metadata: { ...product.metadata, care_instructions: value },
      }),
    onSuccess: (_, value) => {
      setDisplayed(value)
    ...
    onError: () => toast.error("Failed to save care instructions"),
  })
  ...
  return (
    <Container className="flex flex-col gap-4 px-6 py-4">
      ...
      <Text size="small" leading="compact" className="text-ui-fg-subtle whitespace-pre-wrap">
        {displayed || "No care instructions added yet."}
      </Text>

      <Drawer open={open} onOpenChange={setOpen}>
        <Drawer.Content>
          <Drawer.Header>
            <Drawer.Title>Edit Care Instructions</Drawer.Title>
          </Drawer.Header>
          ...
          <Drawer.Body className="flex flex-col gap-4 overflow-auto p-4">
            <div className="flex flex-col gap-2">
              ...
              <Textarea
                placeholder="e.g. Hand wash cold. Do not bleach. Lay flat to dry. Iron on low heat."
                value={form}
